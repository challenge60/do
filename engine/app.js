...
(function(){
  ...
  function buildIcon(size){
    ...
    ctx.fillText((typeof EXAM_CONFIG !== "undefined" && EXAM_CONFIG.markText) ? EXAM_CONFIG.markText.slice(0,4) : "필답", size / 2, size * 0.42);
  ...
  try{
    ...
    const manifest = {
      ...
      short_name: (typeof EXAM_CONFIG !== "undefined" && EXAM_CONFIG.markText) ? EXAM_CONFIG.markText + " 필답노트" : "필답노트",
...
const LS_KEY = "exam_app_v1__" + (typeof CERT_ID !== "undefined" ? CERT_ID : "default");
function loadStore(){
  try{
...
/* ---- 관리자 전용: 문제 수정사항을 전체 사용자 기본값으로 발행(publish) ----
   window.isAdmin / window.publishAdminOverride 는 storage.js(허브에서만 로드)가 채워준다.
   관리자가 아니거나 storage.js가 없는(standalone) 환경에서는 아무 일도 하지 않는다. */
async function maybeAskAdminPublish(id, payload){
  if(typeof window === "undefined" || !window.isAdmin || typeof window.publishAdminOverride !== "function") return;
  const ok = confirm(
...
function buildDailyReviewQueue(){
  ...
  const mix = reviewMix();
  const wrongPool = getData().filter(q=> store.wrong[q.id]);
  const srsPool = getSRSQueue();
  const newPool = getData().filter(q=> isUngraded(q.id));
  if(!goal){
...
function buildShareText(q, withAnswer){
  const multiRound = datasetHasMultiRound(getData());
  const label = yearRoundLabel(q, multiRound) + (q.no?`-${q.no}`:"");
  const lines = [];
  lines.push(`건축기사 실기 필답노트 · ${label}${q.unitMajor?" · "+q.unitMajor:""}${q.unitMinor?" · "+q.unitMinor:""}`);
  lines.push("");
  lines.push("[문제]");
...
async function shareParseSegments(text, images){
  ...
  while((m = boxRe.exec(raw)) !== null){
    ...
    const items = await shareParseInline(m[1], images, usedIdx);
...
function shareLayoutSegments(mctx, segments, contentW, textFont, lineH, maxImgH){
  ...
  return segments.map(seg=>{
    ...
    return {type:"box", items, innerW, h: shareSegmentsHeight(items) + boxPad*2, gap:14, pad:boxPad};
  });
}
...
function shareDrawSegments(ctx, items, x, yStart, contentW, C){
  ...
  items.forEach(item=>{
    if(item.type === "text"){
      ...
      ctx.stroke();
      ctx.restore();
      shareDrawSegments(ctx, item.items, x + item.pad, y + item.pad, item.innerW, C);
      y += item.h + item.gap;
    }
  });
}
...
async function buildQuestionShareCanvas(q, withAnswer){
  ...
  ctx.fillText("건축기사 실기 필답노트", PAD, 44);
  ...
  ctx.textAlign = "center";
  ctx.fillText(`건축기사 실기 필답노트 · Ver.${typeof APP_VERSION!=="undefined"?APP_VERSION:""}`, W/2, H-28);
  ctx.textAlign = "left";
...
async function shareQuestionText(q, withAnswer){
  const text = buildShareText(q, withAnswer);
