...
/* 회독 학습의 "신규 연도 범위" 필터: null이면 전체 연도 대상.
   지정해두면 오답·SRS·신규 세 풀 모두 이 연도 범위 안의 문항으로만 채워진다. */
function reviewYearRange(){
  return (store.reviewYearRange && (store.reviewYearRange.min!=null || store.reviewYearRange.max!=null)) ? store.reviewYearRange : null;
}
function inReviewYearRange(q){
  const r = reviewYearRange();
  if(!r) return true;
  if(r.min!=null && q.year < r.min) return false;
  if(r.max!=null && q.year > r.max) return false;
  return true;
}
...
function renderSRSList(){
      ...
      <div class="rd-detail-wrap ${rdSettingsOpen?'open':''}" id="settingsDetailWrap">
        ...
        <div style="margin-top:20px;padding-top:14px;border-top:1px solid var(--line);">
          ...
          <p style="font-size:0.78rem;color:var(--muted);margin-top:4px;">
            지정하면 오답노트·복습주기·신규문제 세 풀 모두 이 연도 범위 안의 문항으로만 채워져요.
            예: 최근 3~5년 기출 위주로 보고 싶을 때. 비워두면 전체 연도 대상이에요.
          </p>
          <div style="margin-top:10px;display:flex;gap:8px;align-items:center;">
            <input type="number" id="rdYearMinInput" class="search-box" placeholder="시작 연도(예: 2020)" style="max-width:160px;" value="${(store.reviewYearRange&&store.reviewYearRange.min!=null)?store.reviewYearRange.min:''}">
            <span style="color:var(--muted);">~</span>
            <input type="number" id="rdYearMaxInput" class="search-box" placeholder="끝 연도" style="max-width:160px;" value="${(store.reviewYearRange&&store.reviewYearRange.max!=null)?store.reviewYearRange.max:''}">
          </div>
  ...
  app.querySelector("#saveYearRangeBtn").addEventListener("click", ()=>{
    const minVal = app.querySelector("#rdYearMinInput").value.trim();
    const maxVal = app.querySelector("#rdYearMaxInput").value.trim();
    const min = minVal === "" ? null : Number(minVal);
    const max = maxVal === "" ? null : Number(maxVal);
    if(min!=null && max!=null && min>max){ alert("시작 연도가 끝 연도보다 클 수 없어요."); return; }
    store.reviewYearRange = {min, max};
    saveStore();
    renderSRSList();
  });
