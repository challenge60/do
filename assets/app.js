...
(function(){
  ...
  function buildIcon(size){
    ...
    const iconMark = (typeof EXAM_CONFIG !== "undefined" && EXAM_CONFIG.markText) ? EXAM_CONFIG.markText.slice(0, 4) : "필답";
    ctx.fillText(iconMark, size / 2, size * 0.42);
  ...
  try{
    ...
    const cfgForManifest = (typeof EXAM_CONFIG !== "undefined") ? EXAM_CONFIG : {};
    const manifest = {
      name: cfgForManifest.titleText || "자격증 필답 노트",
      short_name: (cfgForManifest.markText ? cfgForManifest.markText + " 필답노트" : "필답노트"),
...
/* 자격증마다 localStorage 키가 겹치면 서로 다른 자격증의 학습기록이 뒤섞인다.
   data.js에서 정의된 CERT_ID를 이용해 자격증별로 키를 분리한다. */
